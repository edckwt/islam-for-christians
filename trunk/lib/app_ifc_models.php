...
class app_ifc_models {
	...
	{
		global $opicifc_categories_lang;
		$return = array();
		$table = $this->wpdb->prefix.'options';
		$like = OPICIFC_Input_SLUG."category_".get_option(OPICIFC_Input_SLUG.'language').'_';
		...
		if(!empty($data) && is_array($data)){
			foreach ($data as $key => $value) {
				...
				$return[$key]['option_url'] = $opicifc_categories_lang[get_option(OPICIFC_Input_SLUG.'language')][$return[$key]['option_slug']]['importurl'];
