...
class app_ifc_cronjob extends app_ifc_models {
    public $pageCountKey ;
	...
	function __construct() {
		parent::__construct();
		$this->pageCountKey =  get_option(OPICIFC_Input_SLUG.'language')."-islam-for-christians-api-all-p-count-";
		$this->curncyPageKey =  get_option(OPICIFC_Input_SLUG.'language')."-islam-for-christians-api-curency-page-";
	}
	...
	public function InsetPost() {
		...
		$ifchtml = new app_ifc_helpers();
		$ifclang = $ifchtml -> class_lang;
		
		$posts = $this -> importUrl();
		
		foreach ($posts as $key => $value) {
			
			$parentname = $ifclang[$key];
...
$opic_ifc_cronjob = new app_ifc_cronjob();
